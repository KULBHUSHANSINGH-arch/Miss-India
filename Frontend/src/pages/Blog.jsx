...
export default function Blog() {
  ...
  // Tabs: All · News · Blog, then any categories in use.
  const cats = useMemo(() => {
    const types = ['News', 'Blog'].filter((t) => (posts || []).some((p) => p.type === t.toLowerCase()));
    return [...new Set(['All', ...types, ...(posts || []).map((p) => p.category)])];
  }, [posts]);
  const shown = (posts || []).filter((p) =>
    cat === 'All' || (cat === 'News' || cat === 'Blog' ? p.type === cat.toLowerCase() : p.category === cat));

  return (
    <>
      <PageHero eyebrow="Blog & news" title="News & stories from the" script="Runway" text="Announcements, grooming tips, behind-the-scenes and winner stories." />
